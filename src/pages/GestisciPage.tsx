...
// (CODICE COMPLETAMENTE RIVISTO CON MODAL PER AGGIUNGERE EVENTI)
...
// MODIFICA: Componente rinominato e terminologia aggiornata
...
// MODIFICA: Nuovo componente per il modal "Aggiungi Evento", copiato e adattato da AziendaPage
                    ...
                    <div className="modal-body" style={{ minHeight: '350px' }}>
                        {currentStep === 1 && <div>
                            <div className="form-group">
                                <label>Nome Evento <span style={{color: 'red', fontWeight:'bold'}}>* Obbligatorio</span></label>
                                <input type="text" name="eventName" value={formData.eventName} onChange={handleInputChange} className="form-input" maxLength={100} />
                                <small className="char-counter">{formData.eventName.length} / 100</small>
                            </div>
                            <div style={helpTextStyle}><p>Inserisci un nome identificativo per questo evento, può essere un'operazione specifica (come "Raccolta", "Trasformazione", "Spedizione"), un controllo di qualità, un passaggio logistico, un aggiornamento contrattuale o qualsiasi attività rilevante per la tracciabilità del prodotto. Scegli un nome descrittivo che aiuti a comprendere subito di cosa si tratta.</p></div>
                        </div>}
                        {currentStep === 2 && <div>
                            <div className="form-group">
                                <label>Descrizione <span style={{color: '#6c757d'}}>Non obbligatorio</span></label>
                                <textarea name="description" value={formData.description} onChange={handleInputChange} className="form-input" rows={4} maxLength={500}></textarea>
                                <small className="char-counter">{formData.description.length} / 500</small>
                            </div>
                            <div style={helpTextStyle}><p>Fornisci una breve descrizione di cosa è avvenuto in questo evento: specifica le attività svolte, i dettagli rilevanti, eventuali soggetti coinvolti o note tecniche utili per la tracciabilità.</p></div>
                        </div>}
                        {currentStep === 3 && <div>
                             <div className="form-group">
                                <label>Luogo <span style={{color: '#6c757d'}}>Non obbligatorio</span></label>
                                <input type="text" name="location" value={formData.location} onChange={handleInputChange} className="form-input" maxLength={100} />
                                <small className="char-counter">{formData.location.length} / 100</small>
                            </div>
                            <div style={helpTextStyle}><p>Inserisci il luogo dove si è svolto questo evento (es. un magazzino, un laboratorio, un punto vendita, ecc.).</p></div>
                        </div>}
                        {currentStep === 4 && <div>
                            <div className="form-group">
                                <label>Data <span style={{color: '#6c757d'}}>Non obbligatorio</span></label>
                                <input type="date" name="date" value={formData.date} onChange={handleInputChange} className="form-input" max={new Date().toISOString().split('T')[0]} />
                            </div>
                            <div style={helpTextStyle}><p>Inserisci la data in cui si è verificato l'evento.</p></div>
                        </div>}
                        {currentStep === 5 && <div>
                            <div className="form-group">
                                <label>Carica Allegato <span style={{color: '#6c757d'}}>Non obbligatorio</span></label>
                                <input type="file" name="attachment" onChange={handleFileChange} className="form-input" accept=".jpg, .jpeg, .png, .webp, .pdf, .docx, .xlsx, .txt, .csv"/>
                                {selectedFile && <p className="file-name-preview">File selezionato: {selectedFile.name}</p>}
                                <small style={{marginTop: '4px'}}>Formati supportati. Max: 5 MB.</small>
                            </div>
                            <div style={helpTextStyle}><p>Allega un documento o un'immagine relativa a questo evento (es. bolla di trasporto, certificato di analisi, foto del prodotto, ecc.).</p></div>
                        </div>}
                    </div>
...
// ... Resto dei componenti come GestisciPageHeader, ImagePlaceholder, BatchSummaryCard ...
// (Li ometto per brevità, ma sono presenti nel codice completo sotto)
...
export default function GestisciPage() {
    ...
    // MODIFICA: Stato per gestire l'apertura del nuovo modal
    ...
    const fetchBatchData = async () => {
        ...
        try {
            ...
            const infoPromise = readContract({ contract, abi, method: "function getBatchInfo(uint256) view returns (uint256,address,string,string,string,string,string,string,bool)", params: [id] });
            const stepCountPromise = readContract({ contract, abi, method: "function getBatchStepCount(uint256) view returns (uint256)", params: [id] });
            const [info, stepCount] = await Promise.all([infoPromise, stepCountPromise]);
            
            setBatchInfo(info);
            
            const stepsPromises = Array.from({ length: Number(stepCount) }, (_, i) => 
    ...
    // MODIFICA: Logica di finalizzazione aggiornata con nuovo popup di conferma
    ...
    const handleAddEventoSuccess = () => {
        ...
        fetchBatchData(); // Ricarica i dati per mostrare il nuovo evento
    };

    // --- Componenti Header, Placeholder, Card principale non modificati ---
    const GestisciPageHeader = ({ contributorInfo }: { contributorInfo: any }) => { /* ... codice invariato ... */ return ( <div className="dashboard-header-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div
><h2>{contributorInfo?.[0]||'Azienda'}</h2><div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}><div className="status-item"><span>Crediti Rimanenti: <strong>{contributorInfo?.[1]?.toString()||'...'}</strong></span></div><div className="status-item" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><span>Stato: <strong>ATTIVO</strong></span><span className="status-icon">✅</span></div></div></div><div><Link to="/"><button style={{backgroundColor: '#6A5ACD', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '8px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold'}}>← Torna alla Pagina Principale</button></Link></div></div> ); };
    const ImagePlaceholder = () => ( /* ... codice invariato ... */ <div style={{width:'150px',height:'150px',flexShrink:0,backgroundColor:'#f0f0f0',border:'1px solid #ddd',borderRadius:'8px',display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',color:'#a0a0a0',textAlign:'center'}}><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" viewBox="0 0 16 16"><path d="M6.002 5.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0z"/><path d="M2.002 1a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2h-12zm12 1a1 1 0 0 1 1 1v6.5l-3.777-1.947a.5.5 0 0 0-.577.093l-3.71 3.71-2.66-1.772a.5.5 0 0 0-.63.062L1.002 12V3a1 1 0 0 1 1-1h12z"/></svg><div style={{fontSize:'0.8rem',marginTop:'5px',fontWeight:'bold'}}>NO IMAGE<br/>AVAILABLE</div></div> );
    const BatchSummaryCard = ({ batchInfo, stepCount, onAddEventoClick, onFinalize }: { batchInfo: any, stepCount: number, onAddEventoClick: () => void, onFinalize: () => void }) => { /* ... codice con terminologia aggiornata ... */ if(!batchInfo)return null;const defaultImageUrl="https://musical-emerald-partridge.myfilebase.com/ipfs/QmNUGt9nxmkV27qF56jFAG9FUPABvGww5TTW9R9vh2TdvB";const imageUrl=batchInfo[7]&&batchInfo[7]!=="N/A"?`https://musical-emerald-partridge.myfilebase.com/ipfs/${batchInfo[7]}`:defaultImageUrl;const isPlaceholder=imageUrl===defaultImageUrl;const isClosed=batchInfo[8];return(<div className="card" style={{marginTop:'1rem',backgroundColor:'transparent',border:'1px solid #8bc4a8',padding:'1.5rem',display:'flex',alignItems:'center',gap:'2rem'}}>{isPlaceholder?<ImagePlaceholder />:<img src={imageUrl} alt="Immagine batch" style={{width:'150px',height:'150px',objectFit:'cover',borderRadius:'8px',flexShrink:0}} />}<div style={{flex:'1 1 40%',minWidth:0}}><h3 style={{fontWeight:'bold',fontSize:'1.75rem',margin:'0 0 0.5rem 0',color:'white'}}>{batchInfo[3]}</h3><p style={{margin:0,color:'#ced4da',fontSize:'0.95rem'}}>{batchInfo[4]||'Nessuna descrizione fornita.'}</p></div><div style={{flex:'1 1 25%',color:'#ced4da',textAlign:'left'}}><p style={{margin:'0.3rem 0'}}><strong>Stato Iscrizione:</strong> <span style={{color:isClosed?'#dc3545':'#28a745',fontWeight:'bold'}}>{isClosed?'Chiuso':'Aperto'}</span></p><p style={{margin:'0.3rem 0'}}><strong>Luogo:</strong> {batchInfo[6]||'N/D'}</p><p style={{margin:'0.3rem 0'}}><strong>Data:</strong> {batchInfo[5]||'N/D'}</p><p style={{margin:'0.3rem 0'}}><strong>N° Eventi:</strong> {stepCount}</p></div>{!isClosed&&(<div style={{display:'flex',flexDirection:'column',gap:'1rem'}}><button onClick={onAddEventoClick} style={{backgroundColor:'#6A5ACD',color:'white',border:'none',padding:'12px 24px',borderRadius:'8px',cursor:'pointer',fontSize:'1rem',fontWeight:'bold',width:'200px'}}>Aggiungi Evento</button><button onClick={onFinalize} style={{backgroundColor:'#495057',color:'#ced4da',border:'none',padding:'12px 24px',borderRadius:'8px',cursor:'pointer',fontSize:'1rem',fontWeight:'bold',width:'200px'}}>Finalizza Iscrizione</button></div>)}</div>);};
        ...
        <div className="app-container-full" style={{ padding: '0 2rem' }}>
            ...
            <main className="main-content-full">
                ...
                {isLoading ? <p style={{textAlign: 'center', marginTop: '2rem'}}>Caricamento dati iscrizione...</p> : 
                    <>
                        <BatchSummaryCard batchInfo={batchInfo} stepCount={eventi.length} onAddEventoClick={() => setIsModalOpen(true)} onFinalize={handleFinalize} />
            ...
            {/* MODIFICA: Render condizionale del nuovo modal */}
            {isModalOpen && batchId && (
                <AggiungiEventoModal 
