...
const BatchTable = ({ batches }: { batches: (BatchMetadata & { localId: number })[] }) => {
  ...
  return (<div className="table-container">
    <table className="company-table">
      ...
      <tbody>{visibleBatches.length > 0 ? (visibleBatches.map(batch => <BatchRow key={batch.id} batchId={batch.batchId} localId={batch.localId} />)) : (<tr><td colSpan={7} style={{ textAlign: 'center' }}>Nessuna iscrizione trovata.</td></tr>)}</tbody>
    </table>
    <div className="pagination-controls">
      {itemsToShow < itemsOnCurrentPage.length && (<button onClick={() => setItemsToShow(prev => Math.min(prev + 10, MAX_PER_PAGE))} className='link-button'>Vedi altri 10...</button>)}
...
const DashboardHeader = ({ contributorInfo, onNewInscriptionClick }: { contributorInfo: readonly [string, bigint, boolean], onNewInscriptionClick: () => void }) => {
  ...
  return (<div className="dashboard-header-card">
    ...
    <div className="status-section">
      <div className="status-item">
        <span>Stato: <strong>ATTIVO</strong></span><span className="status-icon">✅</span>
      </div>
      <div className="status-item">
...
export default function AziendaPage() {
  ...
  const fetchAllBatches = async () => {
    ...
    try {
      ...
      const sorted = results.sort((a, b) => a.batchId > b.batchId ? -1 : 1);
      setAllBatches(sorted.map((b, i) => ({ ...b, localId: i + 1 })));
    } catch (err) {
      console.error("Errore nel caricare i lotti:", err);
    } finally {
  ...
  const handleInitializeBatch = async () => {
    ...
    try {
      ...
      sendTransaction(transaction, {
        ...
        onError: (err) => {
          const readable = err.message.toLowerCase().includes("insufficient funds") ? "Crediti Insufficienti, Ricarica" : "Errore nella transazione.";
          setTxResult({ status: 'error', message: readable });
        }
      });
    } catch (error: any) {
      setIsUploading(false);
  ...
  return (<div className="app-container-full">
    ...
    {modal === 'init' && (<div className="modal-overlay" onClick={() => setModal(null)}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="form-group"><label>Immagine</label><input type="file" name="image" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="form-input" accept="image/png, image/jpeg, image/gif" />{selectedFile && <p className="file-name-preview">File selezionato: {selectedFile.name}</p>}</div>
        </div>
        <div className="modal-footer">
          <button onClick={() => setModal(null)} className="web3-button secondary">Chiudi</button>
          <button onClick={handleInitializeBatch} disabled={isPending || isUploading} className="web3-button">{isUploading ? "Caricamento..." : "Conferma"}</button>
        </div>
      </div>
    </div>)}
    {(isPending || txResult) && (
