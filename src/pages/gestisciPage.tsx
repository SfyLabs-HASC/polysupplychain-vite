...
// --- Client e Contratto (come in AziendaPage) ---
...
// Header della pagina (simile a DashboardHeader ma senza il pulsante "Nuova Iscrizione")
const GestisciPageHeader = ({ contributorInfo }: { contributorInfo: any }) => {
    ...
    return (
        <div className="dashboard-header-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative' }}>
            ...
            {/* Pulsante Nuova Iscrizione non presente */}
        </div>
    );
};

// Card Riepilogativa del Batch
        ...
        <div className="card" style={{ marginTop: '2rem', backgroundColor: '#8bc4a8' }}>
            <div style={{ display: 'flex', gap: '2rem' }}>
                ...
                <div style={{ flex: '1' }}>
                    ...
                    <p>
                        <strong>N° Passaggi:</strong> {stepCount} | <strong>Stato:</strong> {batchInfo[8] ? <span className="status-closed">Chiuso</span> : <span className="status-open">Aperto</span>}
                    </p>
                    {!batchInfo[8] && (
...
export default function GestisciPage() {
    ...
    // Stato per i dati del batch e degli step
    const [batchInfo, setBatchInfo] = useState<any>(null);
    const [steps, setSteps] = useState<any[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    // Funzioni per interagire con lo smart contract
    ...
    const fetchBatchData = async () => {
        ...
        try {
            ...
            // Fetch dettagli di ogni passaggio
            const stepsPromises = [];
            for (let i = 0; i < stepCount; i++) {
                // IPOTESI: La funzione per ottenere i dettagli di uno step si chiama 'getStepDetails'
    ...
    const handleFinalize = () => {
        ...
        // IPOTESI: La funzione per chiudere un batch si chiama 'closeBatch'
        ...
        sendTransaction(transaction, {
            onSuccess: () => {
                ...
                fetchBatchData(); // Ricarica i dati per mostrare lo stato aggiornato
            },
            onError: (err) => setTxResult({ status: 'error', message: `Errore: ${err.message}` })
        });
    };

    // Qui andrà la logica per il modal "Aggiungi Passaggio", per ora solo un alert
    const handleAddStepClick = () => {
        alert("Qui si aprirà il modal per aggiungere un nuovo passaggio.");
        // Implementeremo il modal e la funzione handleAddStep in un secondo momento
    };
        ...
        <div className="app-container-full" style={{ padding: '0 2rem' }}>
            ...
            {(isPending || txResult) && (
                <TransactionStatusModal
                    ...
                    message={isPending ? 'Transazione in corso...' : txResult!.message}
